...
prt_detail()
{
	...
	/*F9  TRANSACTION CODE */
		...
		if(bypass)
		{
			_settextposition( line,4 ); _outtext(F1);
			_settextposition( line,8 ); _outtext(F2);
			_settextposition( line,16 ); _outtext(F3);
			_settextposition( line,25 ); _outtext(F4);
			_settextposition( line,39 ); _outtext(F5);
			if(rent->freeze == '3')
			{
				_settextposition( line,43 );
				_outtext("Deleted see next line.");
			}
			else
			{
				_settextposition( line,43 ); _outtext(F6);
				_settextposition( line,50 ); _outtext(F7);
				_settextposition( line,59 ); _outtext(F8);
			}
			_settextposition( line,66 ); _outtext(F9);
			if(emp_sw[tb_no]>0){
			_settextposition( line,78 ); _outtext(F15);}
		}

	/*    _outtext(" %d",rent->employee);    */
		return(0);
}
